...
namespace {
    ...
    void ApplyAutoCollectWaxPatches() {
        // Legacy wax AOB signature is unverified for 0.34.5: fail closed.
    ...
    void ApplyAutoBurnCandlesPatches() {
        // Legacy candle AOB signature is unverified for 0.34.5: fail closed.
    ...
    void ApplyAutoBurnPlantsPatches() {
        ...
        if (!s_abPlantsPatches.empty()) {
            for (auto& p : s_abPlantsPatches) p.Apply();
    ...
    void RestoreAutoBurnPlantsPatches() {
        for (auto& p : s_abPlantsPatches) if (p.address) p.Restore();
        s_abPlantsPatches.clear();
        s_autoBurnPlants = false;
    }
}

bool IsAutoCollectWaxEnabled() { return s_autoCollectWax; }

void SetAutoCollectWaxEnabled(bool enabled) {
    if (enabled) ApplyAutoCollectWaxPatches();
    else RestoreAutoCollectWaxPatches();
}

bool IsAutoBurnCandlesEnabled() { return s_autoBurnCandles; }

void SetAutoBurnCandlesEnabled(bool enabled) {
    if (enabled) ApplyAutoBurnCandlesPatches();
    else RestoreAutoBurnCandlesPatches();
}

bool IsAutoBurnPlantsEnabled() { return s_autoBurnPlants; }

void SetAutoBurnPlantsEnabled(bool enabled) {
    if (enabled == s_autoBurnPlants) return;
    if (enabled) ApplyAutoBurnPlantsPatches();
    else RestoreAutoBurnPlantsPatches();
}
